...
def brick():
    ...
    >>> from pysarpro.transform import rescale, warp, rotate
    >>> from pysarpro.color import rgb2gray
    >>> from imageio import imread, imwrite
    >>> from pysarpro import img_as_ubyte
    >>> import numpy as np
...
def grass():
    ...
    >>> grass = pysarpro.img_as_ubyte(pysarpro.color.rgb2gray(grass_orig[:512, :512]))
...
def gravel():
    ...
    >>> from pysarpro.transform import resize
    >>> gravel_orig = imageio.imread('Gravel04_col.jpg')
    >>> gravel = resize(gravel_orig, (1024, 1024))
    >>> gravel = pysarpro.img_as_ubyte(pysarpro.color.rgb2gray(gravel[:512, :512]))
...
def palisades_of_vogt():
    ...
    https://gitlab.com/Pol-InSAR/data/-/blob/master/README.md#data.
...
def stereo_motorcycle():
    ...
    the function `pysarpro.transform.downscale_local_mean`. The calibration data
...
def skin():
    ...
    file from the pysarpro datasets repository.
